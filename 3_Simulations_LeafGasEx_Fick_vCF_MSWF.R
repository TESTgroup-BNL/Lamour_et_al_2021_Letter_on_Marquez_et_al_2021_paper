...
for(gcw in seq(0,25*10^-3,0.1*10^-3)){
  res=f.A(PFD=c(0.01:2000),cs=cs,Tleaf=Tleaf,Tair=Tair,RH=RH,model_diff='MSWF',
          gcw=gcw,param=param)
...
cs_Fick=f.A(PFD=PFD,cs=seq(30,2000,1),Tleaf=Tleaf,Tair=Tair,RH=RH,model_diff='Fick',param=param)
simu_cs=data.frame()
for(gcw in seq(0,25*10^-3,0.1*10^-3)){
  res=f.A(PFD=PFD,cs=seq(30,2000,1),Tleaf=Tleaf,Tair=Tair,RH=RH,model_diff='MSWF',
          gcw=gcw,param=param)
  simu_cs=rbind.data.frame(simu_cs,cbind.data.frame(as.data.frame(res),gcw=gcw,cs=seq(30,2000,1)))
}
b=(ggplot(data=simu_cs,
          aes(x=cs,y=A,color=gcw))+geom_point(size=size_p)
   +ylab(expression(italic(A)[n]~mu*mol~m^-2~s^-1))
